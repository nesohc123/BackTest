...
class Single_BackTest:    
    ...
    def fit(self, signal_df, groups: Union[int, None] = None, stock_list: Union[list, None] = None, limits_on_long = False):
        ...
        self.real_ret = self.cum_pnl.dropna().iloc[-1] - 1
        self.exc_ret = self.cum_exc_pnl.dropna().iloc[-1] - 1
        self.real_vol = pnl_series.dropna().std()
        self.exc_vol = exc_pnl_series.dropna().std()
